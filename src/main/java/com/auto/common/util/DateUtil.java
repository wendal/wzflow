...
public class DateUtil {
    ...
    static public SimpleDateFormat MMddYYYY_HHmmss = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.CHINA);
    ...
    public static String long2DateStr(long d) {
        try {
            return DateUtil.MMddYYYY_HHmmss.format(d * 1000);
        } catch (Exception e) {
            e.printStackTrace();
            return "";
        }
...
/*            Calendar rightNow = Calendar.getInstance();
    ...
    */
    ...
    public static String getPreday(int n) {
        return getDateStr(getDay(new java.util.Date(), n));
    }

    public static List<String> getPredays(int n) {
        List<String> list = new ArrayList<String>();
        for (int i = 1; i < n; i++) {
            list.add(getPreday(0-i));
        }
        return list;
    }
    ...
    public static void main(String[] args) {
        System.out.println(getPreday(-1));
    }
}
